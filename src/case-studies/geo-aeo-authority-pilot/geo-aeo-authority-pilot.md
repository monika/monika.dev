...
intro: Testing whether structured data and verified authorship change how AI trusts a site's content — average ranking position improved 24.1% in 30 days.
...
cardResult: "Pilot program result: +11.75% average ranking position improvement from internal linking, +24.1% with verified SME authorship added; average search position improved from 24.27 to 18.43 for pilot URLs."
cardTags:
  - Answer engine optimization
  - Structured data
  - Entity authorship
...
**The Challenge:** As LLM traffic to AdRoll's marketing site increased, and direct human visibility decreased, I wanted to understand if we could build "trust" with AI systems by providing verifiable structural metadata.
...
**The Outcome:** Over 30 days, average search ranking position for our pilot group of URLs improved 24.1%, moving from 24.27 to 18.43, by making our content more legible to machines.
...
*Link equity chart outlining the flow of equity from a highly cited data-rich article to specifically chosen articles and an overall topical hub.*
...
| :---- | :---- | :---- | :---- |
| Phase 1 | Structural linking only (hub-and-spoke) | 15 Days | \+11.75% average ranking position improvement |
| Phase 1 | Daily impression volume | 15 Days | \~+6.4% |
| Phase 2 | Author disambiguation (SME schema) | 30 Days | \+24.1% average ranking position improvement (24.27 → 18.43) |

*Restructuring existing content, without writing anything new, produced a real, measurable improvement in ranking position, and verifying who wrote it nearly doubled that effect.*

The DiD report showed real movement in the pilot. Over the first 15 days, treated pages saw an 11.75% improvement in average ranking position against the control group, along with a roughly 6.4% rise in daily impressions. Once the author disambiguation went live in phase two, that improvement grew to 24.1% over the following 30 days, moving average search position for the treated cluster from 24.27 to 18.43.

## The judgement

I didn't design this pilot as high-risk, and that was deliberate. I wasn’t setting up LLM-specific information similar to the use of IE conditional comments back in the Aughts. Nothing brittle or proprietary. This was all handled with default web architecture, schema.org and JSON-LD, public information, and applied deliberately to our site markup. Worst case, I spent a week finding out that schema enrichment doesn't move the needle in terms of building trust with LLMs in the short-term.
